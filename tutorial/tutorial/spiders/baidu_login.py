...
class BaiduLogin1Spider(Spider):
    name="baidu_login1"
    ...
    def parse(self, response):
        ...
        with open("baidu_homepage_cookies.dat", 'wb') as f:
            f.write("BAIDUID=%s\n" % self.baiduid)
            f.write("BDUSS=%s\n" % self.bduss)
    ...
    def parse_get_home_page_result(self, response):
        if response.status is 200:
            ...
            s = re.search(r'BAIDUID=(?P<bid>([0-9a-zA-Z:=]+))', j)
            self.baiduid = s.group("bid")
    ...
    def parse_login_result(self,response):
        if response.status is 200:
            ...
            if response.body.find("err_no=0") > -1:
                ...
                s = re.search(r'BDUSS=(?P<bduss>([0-9a-zA-Z:=]+))', j)
                self.bduss = s.group("bduss")
