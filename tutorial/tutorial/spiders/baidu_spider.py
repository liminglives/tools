...
__Author__ = "Zheng Liming"
__Doc__ = "baidu sign everyday.";
...
class BaiduSigninSpider(Spider):
    name="baidu_signin"
    start_urls = ['http://index.baidu.com/?tpl=trend&word=auto']
    cookies = None

    def start_requestss(self):
        logging.info("start requests")
        self.get_home_page()
        logging.info("%s, %s", type(self.cookies), str(self.cookies))
        for u in self.start_urls:
            yield scrapy.Request(url=u, headers=COMMON_HEADERS,meta={"cookiejar":self.cookies})

    def parse(self, response):
        #logging.info(str(response.body))
        logging.info("parse here")
        self.get_home_page()
        req = Request(url=self.start_urls[0], headers=COMMON_HEADERS,meta={"cookiejar":self.cookies},callback=self.parse_response)
    ...
    def login(self):
        ...
        for line in baidu_accounts:
            ...
            return Request(BAIDU_LOGIN_URL, headers=COMMON_HEADERS, cookies=cookies, body=postData, method="POST",callback=self.parse_login_result)
    ...
    def parse_login_result(self,response):
        if response.status is 200:
            ...
            logging.info("cookies : %s", str(self.cookies))
